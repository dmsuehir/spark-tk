...
class ClassificationMetricsValue(PropertiesObject):
    ...
    def __init__(self, scala_result):
        ...
        if cm:
            ...
            header = map (lambda x: "Predicted_" + x.title(),  list(cm.columnLabels()))
            row_index = map (lambda x: "Actual_" + x.title(),  list(cm.rowLabels()))
...
def classification_metrics(self, label_column, pred_column, pos_label=None, beta=1.0, frequency_column=None):
    ...
    *   The **confusion_matrix** result is a confusion matrix for a
    ...
    """
    return ClassificationMetricsValue(self._scala.classificationMetrics(label_column,
                                      pred_column,
