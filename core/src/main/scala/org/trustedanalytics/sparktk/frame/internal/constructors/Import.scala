...
object Import {
  ...
  def importCsv(sc: SparkContext,
    ...
    val schemaWithIndex = frameSchema.columns.zipWithIndex

    // If the schema has any datetime columns, parse those values
    val firstDateTimeColumn = frameSchema.columns.indexWhere(col => col.dataType == DataTypes.datetime)
    val mappedRdd = if (firstDateTimeColumn == -1) frameRdd else frameRdd.map(row => {
      val parsedValues = schemaWithIndex.map {
        case (column, index) =>
          column.dataType match {
            case DataTypes.datetime => DataTypes.datetime.parse(row.get(index)).get
    ...
    new Frame(mappedRdd, frameSchema)
  }
